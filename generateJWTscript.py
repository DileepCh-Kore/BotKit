    ...
    def __init__(self, account: Text, user: Text, private_key_file_path: Text,
        ...
        with open(self.private_key_file_path, 'rb') as pem_in:
            ...
            try:
                ...
                self.private_key = load_pem_private_key(pemlines, "enEWmdivV%7$R6&h27&0".encode(), default_backend())
...
def main():
    ...
    with open('procureAssistConfig.json') as f:
        credentials = json.load(f)
    token = JWTGenerator(credentials['snowflakeAccount'], credentials['snowflakeUser'], credentials['privateKeyFilePath'], timedelta(minutes=args.lifetime), timedelta(minutes=args.renewal_delay)).get_token()
    with open('token.txt', 'w') as f:
        f.write(token)
    print('JWT:')
    print(token)
